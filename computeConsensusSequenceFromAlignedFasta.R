...
# if add.suffix.to.seqnames is not NA, append the text.  eg. add.suffix.to.seqnames = "_1" for "ElimDups" format.
computeConsensusSequenceFromAlignedFasta <- function ( input.fasta.file, output.dir = NULL, output.file = NULL, consensus.sequence.name = NA, output.fasta.width = 72, include.full.alignment = FALSE, include.consensus = TRUE, use.sequence.numbers.as.names = FALSE, add.suffix.to.seqnames = NA ) {
    ...
    if( !is.na( add.suffix.to.seqnames ) ) {
        rownames( input.fasta ) <- paste( rownames( input.fasta ), add.suffix.to.seqnames, sep = "" );
    }
...
add.suffix.to.seqnames <- Sys.getenv( "computeConsensusSequenceFromAlignedFasta_addSuffixToSeqNames" );
if( ( nchar( add.suffix.to.seqnames ) == 0 ) || ( add.suffix.to.seqnames == "0" ) || ( toupper( add.suffix.to.seqnames ) == "F" ) || ( toupper( add.suffix.to.seqnames ) == "FALSE" ) || ( toupper( add.suffix.to.seqnames ) == "NA" ) ) {
    add.suffix.to.seqnames <- NA;
}
...
if( file.exists( input.fasta.file ) ) {
    print( computeConsensusSequenceFromAlignedFasta( input.fasta.file, output.dir = output.dir, output.file = output.fasta.file, include.full.alignment = include.full.alignment, include.consensus = include.consensus,  use.sequence.numbers.as.names = use.sequence.numbers.as.names, add.suffix.to.seqnames = add.suffix.to.seqnames ) );
} else {
