...
###################################################
### code chunk number 5: DSPFitter.Rnw:290-1096
###################################################
PFitter <- function (
  infile = args[1],
    ...
    if( TRUE ) {
      ...
      if( be.verbose ) {
        if( .p.values[[ "estimate.p.value" ]] <= 0.05 ) {
            cat( "DSPFitter convolution test: DOES NOT FOLLOW A STAR-PHYLOGENY (", .p.values[[ "estimate.p.value" ]], ")", sep = "", fill = TRUE );
        } else {
            cat( "DSPFitter convolution test: FOLLOWS A STAR-PHYLOGENY (", .p.values[[ "estimate.p.value" ]], ")", sep = "", fill = TRUE );
        }
          ## TODO: REMOVE
        if( .p.values[[ "lower.p.value" ]] <= 0.05 ) {
            cat( "DSPFitter lower convolution test: DOES NOT FOLLOW A STAR-PHYLOGENY (", .p.values[[ "lower.p.value" ]], ")", sep = "", fill = TRUE );
        } else {
            cat( "DSPFitter lower convolution test: FOLLOWS A STAR-PHYLOGENY (", .p.values[[ "lower.p.value" ]], ")", sep = "", fill = TRUE );
        }
          ## TODO: REMOVE
        if( .p.values[[ "upper.p.value" ]] <= 0.05 ) {
            cat( "DSPFitter upper convolution test: DOES NOT FOLLOW A STAR-PHYLOGENY (", .p.values[[ "upper.p.value" ]], ")", sep = "", fill = TRUE );
        } else {
            cat( "DSPFitter upper convolution test: FOLLOWS A STAR-PHYLOGENY (", .p.values[[ "upper.p.value" ]], ")", sep = "", fill = TRUE );
        }
      } # End if be.verbose
...
###################################################
### code chunk number 6: DSPFitter.Rnw:1108-1111
###################################################
...
###################################################
### code chunk number 7: DSPFitter.Rnw:1118-1120
###################################################
