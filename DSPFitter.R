  ...
  MAXIMUM.DS.SAMPLE.SIZE = 250
    ...
    if( DS.lambda == 0 ) {
      ...
      while( !is.finite( .f.at.lowest.nonzero.x ) ) {
          ...
          if( .lowest.nonzero.x > DS.lambda ) {
            warning( ".lowest.nonzero.x increased too much, so it would be higher than DS.lambda. Setting it to DS.lambda" );
            .lowest.nonzero.x <- DS.lambda;
          }         
          .f.at.lowest.nonzero.x <- .f( .lowest.nonzero.x );
      }
