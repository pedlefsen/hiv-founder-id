...
same length as $\vec{y^0}$):
$$
y^1_{1} = \frac{1}{2*y^0_{1}*(y^0_{1}-1)}\\
mvals = (2i) for i in 1 .. length(yvec)
delta[i] = a vector of length 2i , all zeros
for( i in 1:length(yvec) {
  m = 2i
  j = 1 + m/2 = 1 + i
  delta[ j ] = 1
  for( k in 1:m ) {
    y^1_m += \frac{1}{2*x^1_m*x^1_{m-k+1}}
    if( i != length(yvec) ) {
      y^1_m += \frac{1}{2*(x^1_{m-k+2}-\delta_k)}
    }
  }
  if( i != length(yvec) ) {
    y^1_m += \frac{1}{2*x^1_{m+1}*x^1_{1}}
  }

}
$$
, where $\vec{x^1} <- c(\vec{y^0}, rep(0,nl0))$.
