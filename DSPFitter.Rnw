...
if( be.verbose ) {
    ...
    if( pval < 0.05 ) {
        ...
        print( paste( "FITS (insufficient evidence to conclude that it does not).\n", .message, sep = "" ) );
    }
}
...
draw.once.from.each.and.evaluate.assertion <- function( assertion.is.that.1.is.x.times.2 = 2, sorted.observed.data.1 = sorted.consensus.distances, sorted.observed.data.2 = sorted.intersequence.distances ) {
    ...
    .sample1.max = assertion.is.that.1.is.x.times.2 * min( .sample1$lambda.low + .sample1$lambda.width );
    .sample1.min = assertion.is.that.1.is.x.times.2 * max(  .sample1$lambda.low );
    stopifnot( .sample1.min <= .sample1.max );
