...
makeDList <- function ( consensus.distances, intersequence.distances ) {
    ...
    dlist[ 1:length( consensus.distances ), 2 ] <- ;
    return( dlist );
} # makeDList (..)
...
DSPFitter <- function (
  ...
  be.verbose = TRUE
    ...
    draw.once.from.each.and.evaluate.assertion <- function( assertion.is.that.1.is.x.times.2.low = 2, assertion.is.that.1.is.x.times.2.high = assertion.is.that.1.is.x.times.2.low, sorted.observed.data.1 = sorted.consensus.distances, sorted.observed.data.2 = sorted.intersequence.distances, be.verbose = FALSE, weakening.type = NA ) {
