...
create.intersequence.distances.by.convolving.consensus.distances <- function ( table.of.consensus.distances ) {
    ...
    # #n.seqs <- c( 10, 100, 500 );
    # n.seqs <- sum( consensus.distances );
    # weakening.type <- NA;
...
The convolution HD distribution in PFitter $\vec{y^1}$ (to which the HD histogram $\vec{y^0}$
is compared) is computed like so (after initializing it to have the
same length as $l = \vec{y^0}$):
$$
  y^1_{m} =  \frac{1}{2} \left( \left( \sum_{k=1}^{m}{ x^1_k x^1_{m-k+1}}  \right) - x^1_{1+((m-1)/2)}\mathbf{1}_{\{\hbox{$m$ is odd}\}} \right)
$$
, where $\vec{x^1} = \vec{y^0}$ padded with an additional $l$ zeroes.
