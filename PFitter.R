# Paul Edlefsen downloaded this on Oct 4, 2015 from "http://www.hiv.lanl.gov/tmp/POISSON_FITTER/pp429jC8Td/PFitter.R".  (followed link 'Download R Script' from PoissonFitter results page http://www.hiv.lanl.gov/cgi-bin/POISSON_FITTER/v2/pfitter.cgi; it is "v2" ie it is the post-May 2015 version, which uses U-stat based SDs rather than jackknife resampling.  Paul has altered it to make the version that takes multiple sequences in a single fasta entry (ie when sequence names end in _nnn for nnn>1) give consistent outputs to the same information contained in a file with all _nnn == 1, but the U stat-based stdev calculations remain inconsistent; using _nnn > 1 gives results consistent with the PFitter v2 version online at http://www.hiv.lanl.gov/content/sequence/POISSON_FITTER/pfitter.html as of Nov 27, 2015. [ERE I AM  STILL VERIFYING THIS IS TRUE])
#########################################################################
