...
##
##      Or: mkdir Abrahams-2009aa-profillic_identify_founders_resultDir/; perl ./profillic_identify_founders.pl -V -O Abrahams-2009aa-profillic_identify_founders_resultDir/ Abrahams-2009aa/preparedFor_hiv-identify-founders.list > Abrahams-2009aa-profillic_identify_founders_resultDir/profillic-identify-founders.out 
###******************************************************************************
...
use strict;
use vars qw( $opt_D $opt_V $opt_o $opt_O $opt_H $opt_f $opt_R );
...
sub profillic_identify_founders {
  ...
  sub profillic_identify_founders_usage {
    print "\tprofillic_identify_founders [-DV] [-HfR] [-(o|O) <output_dir>] <input_fasta_file1 or file_listing_input_files> [<input_fasta_file2> ...] \n";
    exit;
  }

  # This means -D, -o, -O, -V, -f, -R are ok, but nothin' else.
  ...
  # opt_R means skip the recombination detection step.
  # opt_f means fix hypermutated sequences, instead of removing them.
  # But first reset the opt vars.
  ( $opt_D, $opt_V, $opt_o, $opt_O, $opt_H, $opt_f, $opt_R ) = ();
  if( not getopts('DVo:O:HfR') ) {
  ...
  my $do_hypermutation_detection = !$opt_H;
  my $fix_hypermutated_sequences = $opt_f || 0;
  my $do_recombination_detection = !$opt_R;
  ...
  #    }
    ...
    } # End if $do_hypermutation_detection

    if( $do_recombination_detection ) {
    ...
    } # End if $do_recombination_detection
    ...
    my $force_one_cluster = 0;#( $self_entropy > $self_entropy_threshold );
